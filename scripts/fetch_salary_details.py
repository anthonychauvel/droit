...
DEFAULT_KEYWORDS = ("salaire", "rémunération", "minima")
...
def find_all_stubs(node, keywords=None, max_stubs=1):
    ...
    le début (les minima peuvent être répartis sur plusieurs avenants).
    """
    if keywords is None:
...
def main():
    ...
    ap.add_argument("--max-stubs-par-ccn", type=int, default=1,
                    help="Nombre max de clauses salaire à récupérer par CCN et par run "
    ...
    for i, idcc in enumerate(ok_ids, 1):
        ...
        for stub_node in stubs:
            ...
            stub_node["_type_complement"] = "salaire"
    ...
    print(f"\nTerminé: {n_updated} CCN complétées ({n_stubs_total} clause(s), {n_appels} appel(s)), {n_skipped} sans avenant salaire trouvé, {n_error} erreurs.")
