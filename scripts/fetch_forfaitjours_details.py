...
forfait jours / forfait heures / cadres autonomes -- comble le même trou que
fetch_overtime_details.py mais pour un tout autre sujet : le container KALI de
base ne renvoie que des stubs vides pour ces sections aussi (confirmé en
session le 18/07/2026 : 0 occurrence de "forfait" dans le texte brut de 860K
caractères de la CCN assurances 1672, alors que le forfait jours y existe bien).

Différence avec fetch_overtime_details.py : une clause forfait jours est
souvent éclatée en PLUSIEURS sous-articles (plafond, rachat, entretien,
déconnexion peuvent être dans des sous-sections séparées), donc ce script
récupère TOUS les stubs pertinents d'une section trouvée, pas seulement le
"meilleur" -- sinon on risque de rater le taux de rachat ou la fréquence
d'entretien qui vivent ailleurs que le stub "plafond".

Pour chaque CCN déjà dans output/ccn/<idcc>.json :
1. Trouve la ou les sections "forfait/convention de forfait/cadres autonomes/
   jours de repos" qui n'ont pas de contenu inline (juste un id/titre)
...
Usage:
    python3 fetch_forfaitjours_details.py --ccn-dir output/ccn
"""
import os
import sys
import json
...
# Mots-clés forfait jours -- volontairement plus larges que fetch_overtime_details.py
# car les intitulés varient beaucoup d'une CCN à l'autre (certaines disent
# "convention de forfait", d'autres "cadres autonomes", d'autres "modalités de
# réalisation de missions" comme Syntec)
KEYWORDS = (
    "forfait", "convention de forfait", "cadres autonomes",
    "jours de repos", "réalisation de missions", "autonomie",
    "cadre dirigeant", "cadres dirigeants",
)
...
def find_all_stubs(node, keywords=KEYWORDS, max_stubs=3):
    """Retourne jusqu'à max_stubs (noeud, titre) de stubs forfait-jours trouvés,
    triés du plus récent au plus ancien. Contrairement à fetch_overtime_details.py
    (1 seul stub), on en garde plusieurs car plafond/rachat/entretien/déconnexion
    peuvent être répartis sur des sous-articles distincts."""
    candidates = []
...
def main():
    ...
    ap.add_argument("--max-stubs-par-ccn", type=int, default=3,
                     help="Nombre max de stubs forfait-jours à récupérer par CCN (défaut 3, "
                          "car plafond/rachat/entretien/déconnexion peuvent être séparés)")
    args = ap.parse_args()
    ...
    print(f"Token OK. {len(ok_ids)} CCN à examiner pour complément forfait jours/cadres autonomes.")
    ...
    for i, idcc in enumerate(ok_ids, 1):
        ...
        print(f"[{i}/{len(ok_ids)}] IDCC {idcc}: {len(stubs)} stub(s) forfait-jours trouvé(s)")
        updated_any = False
        for stub_node in stubs:
            ...
            stub_node["_type_complement"] = "forfait_jours"
    ...
    print(f"\nTerminé: {n_updated} CCN complétées ({n_stubs_total} stubs au total), "
          f"{n_skipped} sans clause forfait-jours trouvée, {n_error} erreurs.")
