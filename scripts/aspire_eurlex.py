...
# -*- coding: utf-8 -*-
r"""
ASPIRATION EUR-Lex v4 — via CELLAR, SANS plafond par run, avec commits
...
def main():
    ...
    print("=== Aspiration EUR-Lex v4 (via CELLAR, sans plafond) ===")
    ...
    for idx, c in enumerate(a_faire, 1):
        ...
        if ok_depuis_commit >= COMMIT_TOUS:
            cok, detail = _git_commit.commit_et_push(
                [DEST], "Aspiration EUR-Lex (auto, %d faits) [skip ci]" % (len(faits) + ok))
    ...
    if ok_depuis_commit > 0:
        cok, detail = _git_commit.commit_et_push(
            [DEST], "Aspiration EUR-Lex (auto, final) [skip ci]")
