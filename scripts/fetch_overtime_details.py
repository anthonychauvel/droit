#!/usr/bin/env python3
"""
Complète les CCN déjà téléchargées avec le texte intégral de leur clause
heures sup/majoration/contingent la plus récente -- comble le trou laissé par kaliContIdcc, qui ne
renvoie que des références vides ("stubs") pour les textes attachés.

Pour chaque CCN déjà dans output/ccn/<idcc>.json :
1. Trouve la section "durée du travail/heures sup/contingent/majoration" la plus récente qui n'a
...
Usage:
    python3 fetch_overtime_details.py --ccn-dir output/ccn
"""
import os
import sys
import json
...
DEFAULT_KEYWORDS = ("durée du travail", "heures supplémentaires", "contingent", "majoration")


def find_best_stub(node, keywords=DEFAULT_KEYWORDS):
    """Retourne (noeud, chemin_vers_le_noeud) du meilleur stub heures sup trouvé,
...
def find_all_stubs(node, keywords=None, max_stubs=1):
    ...
    le début (contingent et majorations sont souvent dans des avenants distincts).
    """
    if keywords is None:
...
def main():
    ...
    ap.add_argument("--max-stubs-par-ccn", type=int, default=1,
                    help="Nombre max de clauses heures sup à récupérer par CCN et par run "
    ...
    for i, idcc in enumerate(ok_ids, 1):
        ...
        for stub_node in stubs:
            ...
            stub_node["_type_complement"] = "heures_sup"
    ...
    print(f"\nTerminé: {n_updated} CCN complétées ({n_stubs_total} clause(s), {n_appels} appel(s)), {n_skipped} sans clause heures sup trouvée, {n_error} erreurs.")
